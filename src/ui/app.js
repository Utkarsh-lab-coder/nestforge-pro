  ...
  async _runNestingProceed() {
    ...
    if (tooWide.length) {
      ...
      alert(
        ...
        `Increase sheet width to at least ${minNeeded}mm,\n` +
    ...
    try {
      ...
      if (settings.autoLength && !settings.fillSheet && !settings.multiSheet && !_isHide && settings.copies > 1) {
        ...
        console.log('[NestForge] Auto-expand: width=' + settings.sheetW + 'mm, target=' + _autoExpandTargetCopies + ' copies, infinite height');
      }
      ...
      for (let si = 0; si < strategies.length; si++) {
        ...
        let thisResult = null;
        let thisAttempts = 0;
        // Reset sheet height for each strategy (avoid contamination)
        settings.sheetH = origH;
        document.getElementById('s-height').value = origH;
        Renderer.sheetH = origH;
        ...
        while (thisAttempts < MAX_ATTEMPTS) {
          ...
          const newH = Math.ceil(settings.sheetH * ratio);
          if (newH > 100000) break;
          settings.sheetH = newH <= settings.sheetH ? Math.ceil(settings.sheetH * 1.5) : newH;
          document.getElementById('s-height').value = settings.sheetH;
          Renderer.sheetH = settings.sheetH;
          document.getElementById('prog-sub').textContent =
            `Auto-fit: growing to ${settings.sheetH}mm…`;
          await sleep(0);
          thisAttempts++;
        }
      ...
      while (attempts < MAX_ATTEMPTS) {
        ...
        const newH = Math.ceil(settings.sheetH * ratio);
        if (newH > 100000) break; // safety cap
        if (newH <= settings.sheetH) {
          // Loop made no progress — bump by 50% as a last try
          settings.sheetH = Math.ceil(settings.sheetH * 1.5);
        } else {
          settings.sheetH = newH;
        }
        document.getElementById('s-height').value = settings.sheetH;
        Renderer.sheetH = settings.sheetH;
        document.getElementById('prog-sub').textContent =
          `Auto-fit: growing to ${settings.sheetH}mm  (${result.unplaced} part(s) didn't fit, retrying…)`;
        await sleep(0);
        attempts++;
      }
      ...
      if (!this._cancelled && result && result.unplaced === 0 && !settings.fillSheet
          && !settings.multiSheet && !isHideSheet
